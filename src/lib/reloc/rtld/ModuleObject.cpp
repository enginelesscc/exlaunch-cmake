...
namespace rtld {
    ...
    void ModuleObject::Initialize(char* aslr_base, Elf_Dyn* dynamic) {
        ...
        for (; dynamic->d_tag != DT_NULL; dynamic++) {
            ...
            case DT_HASH: {
                uint32_t* hash_table = (uint32_t*)(aslr_base + dynamic->d_un.d_val);
            ...
            case DT_REL: {
                this->rela_or_rel.rel = (Elf_Rel*)(aslr_base + dynamic->d_un.d_val);
                break;
            }

            case DT_RELA: {
                this->rela_or_rel.rela = (Elf_Rela*)(aslr_base + dynamic->d_un.d_val);
                break;
            }
    ...
    void ModuleObject::Relocate() {
        if (this->rel_count) {
            for (auto i = 0u; i < this->rel_count; i++) {
                ...
                case ARCH_RELATIVE: {
                    Elf_Addr* ptr = (Elf_Addr*)(this->module_base + entry->r_offset);
    ...
    Elf_Sym* ModuleObject::GetSymbolByName(const char* name) {
        ...
        for (uint32_t i = this->hash_bucket[name_hash % this->hash_nbucket_value]; i; i = this->hash_chain[i]) {
            ...
            if (!is_common && strcmp(name, this->dynstr + this->dynsym[i].st_name) == 0) {
    ...
    bool ModuleObject::TryResolveSymbol(Elf_Addr* target_symbol_address, Elf_Sym* symbol) {
        ...
        if (ELF_ST_VISIBILITY(symbol->st_other)) {
            ...
            if (target_symbol) {
                *target_symbol_address = (Elf_Addr)this->module_base + target_symbol->st_value;
                return true;
    ...
    void ModuleObject::ResolveSymbolRelJumpSlot(Elf_Rel* entry, bool do_lazy_got_init) {
        uint32_t r_type = ELF_R_TYPE(entry->r_info);
        uint32_t r_sym = ELF_R_SYM(entry->r_info);
    ...
    void ModuleObject::ResolveSymbolRelaJumpSlot(Elf_Rela* entry, bool do_lazy_got_init) {
        uint32_t r_type = ELF_R_TYPE(entry->r_info);
        uint32_t r_sym = ELF_R_SYM(entry->r_info);
    ...
    void ModuleObject::ResolveSymbols(bool do_lazy_got_init) {
        ...
        if (this->is_rela) {
            if (this->rela_or_rel_plt_size >= sizeof(Elf_Rela)) {
                for (auto index = 0u; index < this->rela_or_rel_plt_size / sizeof(Elf_Rela); index++) {
            ...
            for (auto index = 0u; index < this->rela_or_rel_plt_size / sizeof(Elf_Rel); index++) {
