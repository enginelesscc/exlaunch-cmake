...
void virtmemSetup(void) {
    if (R_FAILED(_GetRegionFromInfo(&g_AddressSpace, InfoType_AslrRegionAddress, InfoType_AslrRegionSize))) {
        ...
        if (rc == 0xD401) {
            ...
            // Wat.
            R_ABORT_UNLESS(MAKERESULT(Module_Libnx, LibnxError_WeirdKernel));
        }
    } else {
        R_ABORT_UNLESS(_GetRegionFromInfo(&g_Region[REGION_STACK], InfoType_StackRegionAddress, InfoType_StackRegionSize));
    }

    R_ABORT_UNLESS(_GetRegionFromInfo(&g_Region[REGION_HEAP], InfoType_HeapRegionAddress, InfoType_HeapRegionSize));
